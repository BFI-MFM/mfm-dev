...
load '/home/vzhorin/mfm/sets/sets-theta-infty_new.mat'
v_infty = v;
theta_infty = theta;
ksi_infty =ksi;
...
f1= figure;plot(mu_y+zfun,'Linewidth',1.6');hold on;
plot(mu_y+zfun+hhfun1_infty(v_infty),'Linewidth',1.6);
plot(mu_y+zfun+hhfun1_20(v_20),'Linewidth',1.6);
xlabel('z',FS,14);
legend1 = legend('original model','\theta=\infty','\theta=20');
...
f2=figure;plot(mu_z-k.*zfun,'Linewidth',1.6);hold on;
plot(mu_z-k.*zfun+hhfun2_infty(v_infty),'Linewidth',1.6);
plot(mu_z-k.*zfun+hhfun2_20(v_20),'Linewidth',1.6);
xlabel('z',FS,14);
legend2 = legend('original model','\theta=\infty','\theta=20');
...
legend1 = legend('-h^*(1),  \theta=\infty',...
	'-h^*(2),  \theta=\infty','-h^*(1), \theta=20','-h^*(2), \theta=20');
set(legend1,FS,14, 'Location','best');
print('-depsc2', 'sets_h.eps')
...
d3 = [ min(hfun1_20(v_20)) max(hfun1_20(v_20))-0.001];
d4 = [ min(hfun2_20(v_20))  max(hfun2_20(v_20))-0.001];
...
legend1 = legend('p(h_1),  \theta=\infty',...
	'p(h_2),  \theta=\infty','p(h_1), \theta=20','p(h_2), \theta=20');
set(legend1,FS,14, 'Location','best');
xlabel('h',FS,14);
print('-depsc2', 'sets_h_density.eps')

clear alpha;
figure; 
h2=area(xx3,g_20_1);hold on;
alpha(0.5);h1 = area(xx1,g_infty1);
legend1 = legend('p(h_1), \theta=20','p(h_1), \theta=\infty');
set(legend1,FS,14, 'Location','best');
xlabel('h',FS,14);
...
h2.EdgeColor = 'none';
print('-depsc2','-loose', 'sets_h1_density.eps')

figure; 
h2=area(xx4,g_20_2);hold on;h1 = area(xx2,g_infty2);
legend1 = legend('p(h_2), \theta=20','p(h_2), \theta=\infty');
set(legend1,FS,14, 'Location','best');
xlabel('h',FS,14);
h1.FaceColor = [0 0.25 0.25];
%h1.EdgeColor = [0 0.25 0.25];
...
%h2.EdgeColor = [0 0.5 0.5];
h2.EdgeColor = 'none';
print('-depsc2','-loose', 'sets_h2_density.eps')