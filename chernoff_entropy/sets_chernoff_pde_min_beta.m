...
% ff1 =ff;
% zz1 =zz;
% load beta_chernoff_set_240.mat;
% ff = squeeze(ff)';
% zz(ff==min(ff))=[];
% ff(ff==min(ff))=[];
% ff2 =ff;
% zz2 =zz;
% load beta_chernoff_set_360.mat;
% ff = squeeze(ff)';
% zz(ff==min(ff))=[];
% ff(ff==min(ff))=[];
% ff3 =ff;
...
for kk=1:n_theta
	...
	w{1} = valfun;
	for i = 2:200
		u = 0.01.*feval(diff(f,1),zz)-diff(valfun,1);
		z_roots = roots(u);
		...
		% 	1/2.*norm(sigma_z).^2.*diff(v,2) - theta_term(v);
 		 ...
 		 theta_term = @(v)  (0.01.*(0.01*ss_sigma(1,1)+diff(valfun,1).*ss_sigma(1,2))+...
			 ...
			 *1./(2.*theta);
		N.op = 	@(v) -delta.*v + 0.01.*(mu_y+f(z_sol).*zfun)+diff(v,1).*(mu_z-z_sol.*zfun)+...
			1/2.*norm(sigma_z).^2.*diff(v,2)- theta_term(v);
