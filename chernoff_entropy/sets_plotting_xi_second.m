...
load '/home/vzhorin/new_ss/sets_density-theta-xi-second_spec-1-2.mat'
v_infty = v;
theta_infty = theta;
ksi_infty =ksi;
...
xlabel('z',FS,14);
legend1 = legend('original model','\theta=\infty','\theta=35');
...
print(gcf,'-depsc2','-loose','sets_drift1_xi-second.eps');
...
xlabel('z',FS,14);
legend2 = legend('original model','\theta=\infty','\theta=35');
set(legend2,FS,14, 'Location','best');
print(gcf, '-depsc2', '-loose','sets_drift2_xi-second.eps')
...
xlabel('z',FS,14);
legend1 = legend('-h^*(1),  \theta=\infty',...
	'-h^*(2),  \theta=\infty','-h^*(1), \theta=35','-h^*(2), \theta=35');
set(legend1,FS,14, 'Location','best');
print('-depsc2', 'sets_h_xi-second.eps')


xgrid = linspace(-3,3,5000);
...
q = exp(logq)/sum(0.5*sum(exp(logq(1:end-1)).*dxc(1:end-1) + exp(logq(2:end)).*dxc(2:end)));
 ...
 xlabel('z',FS,14);
 print('-depsc2','-loose', 'sets_density-second.eps')
...
d3 = [ min(hfun1_20(v_20)) max(hfun1_20(v_20))];
d4 = [ min(hfun2_20(v_20))  max(hfun2_20(v_20))];
...
dinv=[-3,0.89]
zh1 = inv(chebfun(@(x) feval(hfun1_infty(v_infty),x), dinv,  'splitting','on'),'splitting','on');
disp('zh1 computed');
zh2 = inv(chebfun(@(x) feval(hfun2_infty(v_infty),x), dinv,  'splitting','on'),'splitting','on');
disp('zh2 computed');
zh3 = inv(chebfun(@(x) feval(hfun1_20(v_20),x), dinv,  'splitting','on'),'splitting','on');
disp('zh3 computed');
...
% zh4 = inv(chebfun.spline(xx4,feval(hfun2_20(v_20),xx4)),'splitting','on','EPS', 1e-4);
 zh4 = inv(chebfun(@(x) feval(hfun2_20(v_20),x), dinv,  'splitting','on'),'splitting','on');
disp('zh4 computed');
...
legend1 = legend('p(h_1), \theta=35','p(h_1), \theta=\infty');
set(legend1,FS,14, 'Location','best');
xlabel('h',FS,14);
...
h2.EdgeColor = 'none';
print('-depsc2','-loose', 'sets_h1_density_xi-second.eps')

figure; 
h2=area(xx4,g_20_2);hold on;h1 = area(xx2,g_infty2);
legend1 = legend('p(h_2), \theta=35','p(h_2), \theta=\infty');
set(legend1,FS,14, 'Location','best');
xlabel('h',FS,14);
h1.FaceColor = [0 0.25 0.25];
%h1.EdgeColor = [0 0.25 0.25];
...
%h2.EdgeColor = [0 0.5 0.5];
h2.EdgeColor = 'none';
print('-depsc2','-loose', 'sets_h2_density_xi-second.eps')
...
legend2 = legend('\theta=\infty','\theta=35');
set(legend2,FS,14, 'Location','best');
print(gcf, '-depsc2', '-loose','sets_value_diff_xi-second.eps')
